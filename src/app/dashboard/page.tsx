  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1 max-w-2xl font-medium">
            Real-time pipeline metrics, opportunity breakdown, and active sales outreach performance.
          </p>
        </div>

        <Link
          href="/"
          className="px-6 py-3 rounded-2xl neu-btn-primary text-white text-xs font-bold flex items-center gap-2 transition-all w-fit cursor-pointer tracking-wide shadow-md"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="p-6 rounded-3xl neu-raised bg-white space-y-2 border border-[#e2e8f0]">
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-slate-500 uppercase tracking-wider font-extrabold">Total Saved Leads</span>
            <div className="w-8 h-8 rounded-xl neu-inset flex items-center justify-center text-sky-600">
              <Users className="w-4 h-4" />
            </div>
          </div>
          <p className="text-3xl font-extrabold text-slate-900 tracking-tight">{totalLeads}</p>
          <p className="text-[11px] text-slate-500 font-semibold">In CRM pipeline</p>
        </div>

        <div className="p-6 rounded-3xl neu-raised bg-white space-y-2 border border-[#e2e8f0]">
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-emerald-600 uppercase tracking-wider font-extrabold">High Opportunity</span>
            <div className="w-8 h-8 rounded-xl neu-inset flex items-center justify-center text-emerald-600">
              <Zap className="w-4 h-4" />
            </div>
          </div>
          <p className="text-3xl font-extrabold text-emerald-600 tracking-tight">{highOppLeads}</p>
          <p className="text-[11px] text-slate-500 font-semibold">{noWebsiteLeads} with zero website</p>
        </div>

        <div className="p-6 rounded-3xl neu-raised bg-white space-y-2 border border-[#e2e8f0]">
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-sky-600 uppercase tracking-wider font-extrabold">Outreach Active</span>
            <div className="w-8 h-8 rounded-xl neu-inset flex items-center justify-center text-sky-600">
              <PhoneCall className="w-4 h-4" />
            </div>
          </div>
          <p className="text-3xl font-extrabold text-sky-600 tracking-tight">{contactedLeads}</p>
          <p className="text-[11px] text-slate-500 font-semibold">{interestedLeads} marked interested</p>
        </div>

        <div className="p-6 rounded-3xl neu-raised bg-white space-y-2 border border-[#e2e8f0]">
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-purple-600 uppercase tracking-wider font-extrabold">Closed Deals</span>
            <div className="w-8 h-8 rounded-xl neu-inset flex items-center justify-center text-purple-600">
              <Award className="w-4 h-4" />
            </div>
          </div>
          <p className="text-3xl font-extrabold text-purple-600 tracking-tight">{closedLeads}</p>
          <p className="text-[11px] text-slate-500 font-semibold">{winRate}% conversion rate</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 p-6 sm:p-8 rounded-3xl neu-raised-lg bg-white space-y-5 border border-[#e2e8f0]">
        ...
        <div className="p-6 sm:p-8 rounded-3xl neu-raised-lg bg-white space-y-4 flex flex-col justify-between border border-[#e2e8f0]">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
            ...
            <div className="space-y-3">
              {campaigns.slice(0, 3).map((camp) => (
                <div key={camp.id} className="p-3.5 rounded-2xl neu-inset space-y-1">
          ...
          <Link
            href="/my-leads"
            className="w-full py-3 rounded-2xl neu-btn text-slate-800 text-xs font-extrabold flex items-center justify-center gap-2 transition-all cursor-pointer"
          >
      ...
      <div className="neu-raised-lg rounded-3xl overflow-hidden space-y-4 p-6 sm:p-8 bg-white border border-[#e2e8f0]">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-extrabold text-slate-900 tracking-wide">Recent High-Opportunity Prospects</h3>
          <Link href="/my-leads" className="text-xs text-sky-600 hover:underline flex items-center gap-1 font-bold">
            <span>Manage all {totalLeads} leads</span>
            <ArrowRight className="w-3.5 h-3.5" />
          </Link>
        </div>

        {leads.length === 0 ? (
          <p className="text-xs text-slate-500 py-4 font-medium">No leads saved yet. Use Lead Finder to discover prospects.</p>
        ) : (
